...
app.get('/worker', async (req, res) => {
  ...
  try {
    ...
    let user = await userModel.exec()
    res.render('worker', { title: 'Restaurant Management System', loginUser: loginUser, records_two: twodata, records_four: fourdata, records_six: sixdata, passcat: pass, items: item, users:user });
  }
  catch (err) {
    throw Error();
  }
...
app.get("/admin", async (req, res) => {
  ...
  try {
    ...
    let item = await getitems.exec()
    let user = await userModel.exec()
    res.render('admin', { title: 'Restaurant Management System', loginUser: loginUser, records_two: twodata, records_four: fourdata, records_six: sixdata, passcat: pass, items: item, users:user });
  }
  catch (err) {
    throw Error();
  }
...
app.post("/add-two", function (req, res, next) {
  ...
  passcatDetails.save(function (err, doc) {
    if (err) throw err;
    res.render('add-two', { title: 'Restaurant Management System', loginUser: loginUser, errors: '', success: 'Product category inserted successfully' });
  })
})
...
app.post("/add-four", function (req, res, next) {
  ...
  passcatDetails.save(function (err, doc) {
    if (err) throw err;
    res.render('add-four', { title: 'Restaurant Management System', loginUser: loginUser, errors: '', success: 'Product category inserted successfully' });
  })
})
...
app.post("/add-six", function (req, res, next) {
  ...
  passcatDetails.save(function (err, doc) {
    if (err) throw err;
    res.render('add-six', { title: 'Restaurant Management System', loginUser: loginUser, errors: '', success: 'Product category inserted successfully' });
  })
})
...
app.post('/password-detail/edit/:id', checkLoginUser, function (req, res, next) {
  ...
  itemModel.findByIdAndUpdate(id, { pass_cat: pass_cat, name: name, price: price, details: details }).exec(function (err) {
    ...
    getPassDetails.exec(function (err, data) {
      ...
      const getPassCat = catMenu.find({})
      getPassCat.exec(function (err, data1) {
        res.render('editItem', { title: 'Restaurant Management System', loginUser: loginUser, records: data1, record: data, success: 'Product Updated Successfully' });
      });
    });
  });
});
...
app.post("/cart", (req, res) => {
  ...
  const username = req.body.custId;
  console.log(username)
  //  var userDetails=new Summary({
  //      username:username,
  //      email:email,
  //      phonenumber:phonenumber,
  //      password:password,
  ...
  //      userDetails.save((err,doc)=>{
  //      if(err) throw err;
  res.render('cart', { title: 'Restaurant Management System', msg: "" });
  // })  ;
